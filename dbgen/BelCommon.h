...
namespace tritium {
    ...
    protected:
        ...
        inline Pip& mk_pip(const data::string& name) { return mk_pip(name,bel.start); }
        Wire& mk_intwire(const data::string& name, vec2 loc);
        inline Wire& mk_intwire(const data::string& name) { return mk_intwire(name, bel.start); }
        BelPin& mk_pin(const data::string& name, BelPin::PinType type, vec2 loc);
        inline BelPin& mk_pin(const data::string& name,BelPin::PinType type) { return mk_pin(name,type, bel.start); }
        ...
        void lnk_termini(a &src, b &dst) {
            ...
            wire.name.push_back(dev.id(fmt::format("X{}Y{}",src.loc.x,src.loc.y)).idx);
            wire.name.push_back(dev.id(fmt::format("X{}Y{}",dst.loc.x,dst.loc.y)).idx);
