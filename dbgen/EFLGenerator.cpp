...
void tritium::EFLGenerator::generate() {
    ...
    lnk_to_wire(omuxshort,out_swire_for_dir(Wire::WireDirection::NORTH));
    lnk_to_wire(omuxshort,out_swire_for_dir(Wire::WireDirection::EAST));
    lnk_to_wire(omuxshort,out_swire_for_dir(Wire::WireDirection::WEST));
    lnk_to_wire(omuxshort,out_swire_for_dir(Wire::WireDirection::SOUTH));
    auto omuxnorth = mk_pip("OMUXNORTH");
    lnk_termini(rtmuxns,omuxnorth);
    lnk_termini(opin_mux,omuxnorth);
    lnk_to_wire(omuxnorth,out_lwire_for_dir(Wire::WireDirection::NORTH));
    auto omuxsouth = mk_pip("OMUXSOUTH");
    lnk_termini(rtmuxns,omuxsouth);
    lnk_termini(opin_mux,omuxsouth);
    lnk_to_wire(omuxsouth,out_lwire_for_dir(Wire::WireDirection::SOUTH));
    auto omuxwest = mk_pip("OMUXWEST");
    lnk_termini(rtmuxns,omuxwest);
    lnk_termini(opin_mux,omuxwest);
    lnk_to_wire(omuxwest,out_lwire_for_dir(Wire::WireDirection::WEST));
    auto omuxeast = mk_pip("OMUXEAST");
    lnk_termini(rtmuxew,omuxeast);
    lnk_termini(opin_mux,omuxeast);
    lnk_to_wire(omuxeast,out_lwire_for_dir(Wire::WireDirection::EAST));
}
